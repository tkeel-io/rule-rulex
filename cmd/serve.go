...
import (
	...
	"github.com/tkeel-io/rule-rulex/internal/bootstrap"
	"github.com/tkeel-io/rule-rulex/internal/conf"
	"github.com/tkeel-io/rule-rulex/pkg/version"
)

func init() {
	mainCmd.PersistentFlags().StringVarP(&metricsBackend, "metrics", "m", "expvar", "Metrics backend (expvar|prometheus)")
...
var mainCmd = &cobra.Command{
	Use:   "serve",
	Short: "run router service.",
	Long:  `run router service.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		srv := bootstrap.NewServer(ctx, cfg, tracing.GlobalTracer(), logger)
		...
		defer func() {
			err := srv.Close()
			if err != nil {
				logger.Bg().Error("server close failure, ", logf.Error(err))
			}
		}()
